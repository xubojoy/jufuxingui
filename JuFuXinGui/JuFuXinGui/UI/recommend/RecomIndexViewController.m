...
- (void)viewWillAppear:(BOOL)animated{
    ...
    self.huodongArray = [NSMutableArray new];
    [self initHeader];
    [self initUIView];
    [self initBtn];
    [self initCircleView];
...
-(void)initScrollView{
    ...
    for (int i = 0; i < 3; ++i) {
        NSDictionary *dic = [self.huodongArray objectAtIndex:i];
...
-(void)mainScrollViewTap{
    ...
    self.mainScorllView.TapActionBlock = ^(NSInteger pageIndex){
        NSLog(@"点击了第%ld个",pageIndex);
         NSDictionary *dic = [rivc.huodongArray objectAtIndex:pageIndex];
        ActivityViewController *avc = [[ActivityViewController alloc] init];
        avc.huodongDic = dic;
        [rivc.navigationController pushViewController:avc animated:YES];
    };
}
...
- (void)goOtherView:(UIButton *)sender{
    ...
    NSString *interestrates = [self.dataDic valueForKey:@"interestrates"];
    RecommendViewController * fifth = [[RecommendViewController alloc]initWithProductId:productId investMenoy:nil interestrates:interestrates];
