...
{
    [super viewDidLoad];
    self.headeriamge=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0,60,60)];
    self.headeriamge.center=CGPointMake((screen_width)/2, _tipLable.center.y+70);
    if (self.user.userimage) {
        //self.headeriamge.image=[UIImage imageNamed:@"head"];
    }
    self.headeriamge.image=[UIImage imageNamed:@"head"];
    self.headeriamge.layer.cornerRadius=30;
    self.headeriamge.layer.masksToBounds=YES;
    [self.view addSubview:self.headeriamge];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //self.savedPassword = [LLLockPassword loadLockPasswordwithusername:self.userid];
...
- (void)checkPassword:(NSString*)string
{
    ...
    if ([string isEqualToString:self.savedPassword]) {
        ...
        if (_nLockViewType == LLLockViewTypeModify) { // 验证旧密码
          ...
          // [LLLockPassword saveLockPassword:nil andusername:self.userid];
            [self hide];
    ...
    // 验证密码错误
        ...
        if (nRetryTimesRemain > 0) {
            ...
            [self goheh];
            //[LLLockPassword saveLockPassword:nil andusername:self.userid];
           // [self showmyAlert:@"超过最大次数，这里该做一些如强制退出重设密码等操作"];
...
- (void)createPassword:(NSString*)string
{
    ...
    // 确认输入密码
        ...
        if ([self.passwordNew isEqualToString:self.passwordconfirm]) {
           ...
           // [LLLockPassword saveLockPassword:string andusername:self.userid];
...
- (void)hide
{
    switch (_nLockViewType) {
        ...
        case LLLockViewTypeModify:
        {
            ...
            //[LLLockPassword saveLockPassword:self.passwordNew andusername:self.userid];
        }
            break;
        case LLLockViewTypeClean:
        default:
...
-(void)goheh{
    ...
    [window.rootViewController presentViewController:logvc animated:NO completion:nil];
