...
-(void)initbody{
    ...
    if (self.local.yonghu==nil) {
        self.local.yonghu=self.context;
    }
    
    webview.scrollView.showsHorizontalScrollIndicator=YES;
    [webview loadHTMLString:self.local.yonghu baseURL:nil];
...
-(void)getadata{
    ...
    __weak YongHuViewController * weeksafa=self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        
        NSString * string=@"http://www.gmtxw06.com/index.php?m=Article&a=user_list";
